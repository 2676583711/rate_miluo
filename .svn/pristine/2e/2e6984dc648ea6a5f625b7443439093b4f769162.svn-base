    ...
    $('#exampleTable').bootstrapTable({
        ...
        onPostBody: function(){
            if(roleId != 1 && roleId != 77){
                $("#exampleTable tbody tr").find('td:eq(9) a:lt(3)').removeAttrs('onclick');
                $("#exampleTable tbody tr").find('td:eq(9) a:lt(3)').click(function () {
...
function updateReturn(id) {
    layer.confirm('确定要把状态改为未处理吗？', {
        ...
        $.ajax({
            url: "/index/polluteWaterUpdateReturn",
            type: "post",
            data: {
                'id': id
            },
...
function updateIgnore(id) {
    ...
    $.ajax({
        url: "/index/polluteWaterUpdateIgnore",
        type: "post",
        data: {
            'id': id
        },
...
function add(id) {
    layer.open({
        ...
        shadeClose : false,
        area : [ '800px', '480px' ],
        content : '/index/polluteWaterAdd?id=' + id
    });
}
function reload() {
...
function exportExcel() {
    layer.confirm('确定要导出当前记录？', {
        ...
        layer.close(index);
        var url = "";
        url += "/index/exportExcel?type=polluteWater&startDate=" + $("#startDate").val()
            + "&endDate=" + $("#endDate").val()
