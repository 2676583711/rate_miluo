...
var Windy = function Windy(params) {
	...
	var VELOCITY_SCALE = 0.010 * (Math.pow(window.devicePixelRatio, 1 / 3) || 1); // scale for wind velocity (completely arbitrary--this value looks nice)
	// var MIN_TEMPERATURE_K = 261.15; // step size of particle intensity color scale
	var MIN_TEMPERATURE_K = 210; // step size of particle intensity color scale
	var MAX_TEMPERATURE_K = 100; // wind velocity at which particle intensity is maximum (m/s)
	...
	// var PARTICLE_MULTIPLIER = 2 / 200; // particle count scalar (completely arbitrary--this values looks nice)
	var PARTICLE_MULTIPLIER = 2 / 400; // particle count scalar (completely arbitrary--this values looks nice)
   ...
   */
	...
	var animate = function animate(bounds, field, extent) {
		...
		function windTemperatureColorScale(minTemp, maxTemp) {
		    //修改风场的风的颜色(解决【影像】地图模式下看不清风的问题) changed on 2018.05.31 by Sunlz
            //原始颜色为蓝色
...
L.Control.WindPosition = L.Control.extend({
	...
	_onMouseMove: function _onMouseMove(e) {
		...
		if (gridValue && !isNaN(gridValue[0]) && !isNaN(gridValue[1]) && gridValue[2]) {
		    ...
		    //htmlOut = "<strong>风向: </strong>" + self.vectorToDegrees(gridValue[0], vMs) + "°" + ", <strong>风速: </strong>" + self.vectorToSpeed(gridValue[0], vMs).toFixed(1) + "m/s";
			htmlOut = "<strong>风速: </strong>" + self.vectorToSpeed(gridValue[0], vMs).toFixed(1) + "m/s";
		} else {
			htmlOut = "没有数据";
		}
...
(function (root, factory) {
   ...
   */
		...
		_loadLocalData: function _loadLocalData() {
			...
			//$.getJSON('Wind/demo/2018010418.json', function (data) {
