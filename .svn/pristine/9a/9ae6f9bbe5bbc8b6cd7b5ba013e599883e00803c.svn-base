...
var summerFresh = {
    ...
    */
    dataService: function (url, data, callback, method, async) {
        ...
        prefixSet.push({ prefix: 'exec:', url: '/Home/ExecCommandBySqlId' });
        prefixSet.push({ prefix: 'crud:', url: 'Grid/ExecCRUDService.json' });
...
//                alert("请求处理异常:" + res.responseText);
    ...
    */
    ...
    showError: function (errorInfo, errorTitle) {
        ...
        summerFresh.showInfo(errorInfo, "close", "异常提示", 300, 200);
    },
    ...
    showInfo: function (info, type, title, width, height, footerHtml) {
        ...
        html += "          <img src=\"content/themes/default/images/icon-" + type + ".png\" />";
        html += "          <span>" + info + "</span>";
    ...
    showSelect: function (selectType, showType, targetValueId, targetTextId, isMultipleSelect, height, width) {
        ...
        var selectUrl = 'BSDStationTypeSelector.html';
    ...
    */
    ...
    /*
        ...
        var parameter={id:3,name:'hello'};
        */
    openPostWindow: function (url, param) {
        ...
        for (var item in param) {
            ...
            hideInput.name = item; //后台要接受这个参数来取值
            hideInput.value = param[item]; //后台实际取到的值
    ...
    widget: {
        ...
        initEchart: function () {
            $("div[widget='Echarts']").each(function (e) {
                ...
                summerFresh.dataService("Grid/GetChartOption.json", settting, function (res) {
        ...
        initNewChart: function () {
            $("div[widget='NewChart']").each(function (e) {
                ...
                summerFresh.dataService("Grid/GetChartOption.json", settting, function (chartOptions) {
        ...
        initChart: function (cId, onFinish) {
            $("div[widget='Chart']").each(function (e) {
                ...
                summerFresh.dataService("Grid/GetChartOption1.json", setting, function (res) {
                    if (onFinish) {
                        if (typeof (onFinish) == 'function') {
                            onFinish(res);
                        }
        ...
        initSearch: function () {
            $("[searchForm]").on("click", "#btnExport", function (e) {
                ...
                else if ($("#sdtDate").length > 0 && $("#edtDate").length > 0) {
                    time = $("#sdtDate").val() + "至" + $("#edtDate").val();
                }
                var fileName = title;
                if (time != '') {
                    fileName += "_" + time;
                ...
                //    return;
                var td = $("td:contains(暂无相关数据)");
                if (td.length > 0) {
                    summerFresh.showError('暂无相关数据，不能导出！');
                    return;
                }
                var e = $.Event("beforeExport");
                table.trigger(e);
                var param = { id: table.attr("componentId"), fileName: fileName };
                param = $.extend({}, param, summerFresh.widget.setting(tableId));
                if (e && e.data) {
                    param = $.extend({}, param, e.data);
                }
                summerFresh.openPostWindow("/Entity/ExportToExcel", param);
            });
        ...
        initCalendar: function () {
            ...
            if (container.length > 0) {
                ...
                summerFresh.dataService("Grid/GetChartOption.json", setting, function (res) {
