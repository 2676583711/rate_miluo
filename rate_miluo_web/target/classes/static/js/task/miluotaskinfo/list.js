...
function exportExcel() {
    layer.confirm('确定要导出当前记录？', {
        ...
        layer.close(index);
        var url = "";
        url += "/task/agents/minExp?beginTime=" + $("#beginTime").val()
            + "&endTime=" + $("#endTime").val()
        ...
        .bootstrapTable(
            {
                ...
                url: prefix + "/minuteList", // 服务器数据的加载地址
                // showRefresh : true,
                // showToggle : true,
                // showColumns : true,
                iconSize: 'outline',
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        formatter: function (value, row, index) {
                            ...
                            var a = '<a class="btn btn-primary btn-sm" href="#" mce_href="#" title="编辑" onclick="editTask(\''
                                + row.id+"\',\'"+row.status
...
function editTask(id,status) {
	layer.open({
		...
		shadeClose : false,
		area : [ '100%', '100%' ],
		content : prefix + '/editTask/' + id+"/"+status // iframe的url
	});
}
