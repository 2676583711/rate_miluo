var prefix = '/statistic/comprehensive';
var flag = true;
var qualityList= [];
var siteCodes="";
var index = 0;
...
function minute() {
    $("#dss4").attr("style","display:show();");
    $("#ds4").attr("style","display:show();");
    $("#main").hide();
...
function initTime() {
    $.ajax({
        url: "/statistic/excellent/initTime",
        type: "post",
        data: {
...
function hourTime() {
    ...
    $("#startMonth").show();
    $("#endMonth").show()
    $("#ds4").attr("style","display:none;");
    $("#dss4").attr("style","display:none;");
    $("#main2").show();
...
function tree() {
    var el;
    var da = $("input[type='radio'][name='stationType']:checked").val(); //获取选中的radio的值 切换树
    if (da == "") {
        $(".siteTree").addClass("d-none");
        $("#siteTree1").removeClass("d-none");
        el = $("#siteTree1");
        $.ajax({
            type: "GET",
            url: "/site/tree/allTree",
            data: {siteType: "all"},
        ...
        el.on("ready.jstree", function(e, data){
            var inst = data.instance;
            inst.open_node("#small");
            // inst.select_node($('li[aria-level="2"]:eq(0)'));
            var obj=inst.get_node(e.target.firstChild.firstChild.lastChild);
            inst.select_node(obj);
            siteCodes = $("#siteTree2").jstree().get_bottom_selected();
        });
    } else if (da == 2) {
        $(".siteTree").addClass("d-none");
        $("#siteTree3").removeClass("d-none");
        el = $("#siteTree3");
        $.ajax({
            type: "GET",
            url: "/site/tree/allTree",
            data: {siteType: "province"},
            success: function (tree) {
                el.jstree({
                    'core': {
                        'data': tree
        ...
        el.on("ready.jstree", function(e, data){
            var inst = data.instance;
            inst.open_node("#province");
            // inst.select_node($('li[aria-level="2"]:eq(0)'));
            var obj=inst.get_node(e.target.firstChild.firstChild.lastChild);
            inst.select_node(obj);
            siteCodes = $("#siteTree3").jstree().get_bottom_selected();

        });
    } else  if(da == 3){
        $(".siteTree").addClass("d-none");
        $("#siteTree4").removeClass("d-none");
        el = $("#siteTree4");
        $.ajax({
            type: "GET",
            url: "/site/tree/allTree",
            data: {siteType: "country"},
            success: function (tree) {
                el.jstree({
                    'core': {
                        'data': tree
        ...
        el.on("ready.jstree", function(e, data){
            var inst = data.instance;
            inst.open_node("#country");
            // inst.select_node($('li[aria-level="2"]:eq(0)'));
            var obj=inst.get_node(e.target.firstChild.firstChild.lastChild);
            inst.select_node(obj);
            siteCodes = $("#siteTree4").jstree().get_bottom_selected();
        });
    }else if(da == 4){
        $(".siteTree").addClass("d-none");
        $("#siteTree5").removeClass("d-none");
        el = $("#siteTree5");
        $.ajax({
            type: "GET",
            url: "/site/tree/allTree",
        ...
        el.on("ready.jstree", function(e, data){
            ...
            var obj=inst.get_node(e.target.firstChild.firstChild.lastChild);
            inst.select_node(obj);
            siteCodes = $("#siteTree5").jstree().get_bottom_selected();
        });
    }
}
...
function exportExcel() {
    ...
    if(index == 0){
        ...
        layer.confirm('确定要导出当前记录？', {
            ...
            var g = "&endDay=" +$("#endDay").val()+"&jd1=" + $("#jd1").val()+"&jd2=" +$("#jd2").val() ;
            var h = "&siteCodes=" + getSiteCodes().join(",");
            window.location.href = url+a+e+f+g+h;
        })
