...
function load(roleId) {
    $('#exampleTable').bootstrapTable({
        ...
        url: "/index/polluteFactorySituation",
        // showRefresh : true,
        // showToggle : true,
        // showColumns : true,
        iconSize: 'outline',
        ...
        columns: [{
            ...
            title: '24H均值',
            align: 'center',
            field: 'value24H', formatter: function (data, row, index) {
                debugger
             var s=   row.indexValue;
                if (data==null || data==undefined){
                    var e = '<a mce_href="#" style="color:red" title="查看详情"><i>'+"-"+'</i></a> ';
                    return e
                } else {
                 var a=   parseFloat(s);
                    var b=   parseFloat(data);
             if(b>a){
                 var e = '<a mce_href="#" style="text-decoration:underline;color:red" title="查看详情" onclick="eventDetail(\''
                     + row.id+","+row.dataTime+","+row.pollutant
                     + '\')"><i>'+data+'</i></a> ';

                 return e
             }else {
                 var e = '<a mce_href="#" style="text-decoration:underline;color:green" title="查看详情" onclick="eventDetail(\''
                     + row.id+","+row.dataTime+","+row.pollutant
                     + '\')"><i>'+data+'</i></a> ';
...
function lookVideo(id) {
    layer.open({
        ...
        shadeClose : false,
        area : [ '800px', '480px' ],
        content : '/index/polluteDataVideo/'+id // iframe的url
    });
}
...
//		shift : -1,
        ...
        area : [  '50%', '80%' ],
        content: '/index/edetail/' + id // iframe的url
    });
    // layer.full(page);
}
...
function exportExcel() {
    layer.confirm('确定要导出当前记录？', {
        ...
        layer.close(index);
        var url = "";
        url += "/index/exportSituationExcel?type=pollute&startDate=" + $("#startDate").val()
            + "&endDate=" + $("#endDate").val()
