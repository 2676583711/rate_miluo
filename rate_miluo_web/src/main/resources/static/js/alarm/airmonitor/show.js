...
function add(id) {
    layer.open({
        type :2 ,
        title : '代办任务查看',
         // data: {"fzr":fzr,"phone":phone},
        maxmin : true,
        shadeClose : false,
        area : [ '800px', '480px' ],
        // content : '/index/add/'+name+"/"+siteCode+"/"+id +"/"+pollutant+"/"+exceedsBidDate+"/"?fzr=+fzr
        content : '/index/add?id=' + id
    });
}
...
function exportExcel() {
    layer.confirm('确定要导出当前记录？', {
        ...
        layer.close(index);
        var url = "";
        url += "/index/exportExcel?type=air&startDate=" + $("#startDate").val()
            + "&endDate=" + $("#endDate").val()
