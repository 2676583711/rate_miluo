...
polluteFactoryAlarmQueryEpx
===
     SELECT 
      @pageTag(){
           a.id,s.name site_name,v.name equipment_name,v.equment_id id ,
           CONCAT(a.`name`,'[', a.pollutant,']') pollutant,
           a.data_time,a.value,a.index_value,
           a.degree_text,a.text ,a.status
      @}
...
===
           select 
            ...
            from( select * from miluo_site t where  t.site_type =3 and t.status=1 )t2
...
polluteFactoryExceedId
===
   
     select *from miluo_exceeds_bid where id=#id#


updatePolluteFactoryTask      
===
...
selHourPollutant
===
     SELECT ROUND(avg(#text(pollutant)#),2) siteName FROM  miluo_pollutant_hour 
     where equment_id = #siteId# and recording_time >=#startTime# and recording_time <=#endTime# and #text(pollutant)# >0 and  data_type ='1'
	
selHourPollutantList
===
      SELECT
         @pageTag(){
       p.#text(pollutant)# so2 , p.recording_time ,  s.name site_name 
       @}
       FROM  miluo_pollutant_hour  p
       left join miluo_video v on v.equment_id = p.equment_id
              left join miluo_site s on v.site_id = s.id
         where p.equment_id = #siteId# and recording_time >=#startTime# and recording_time <=#endTime# and  p.data_type ='1'
